...
function ListItemBlog({ blog, className }: { blog: Blog; className?: string }) {
	return (
		<article
			key={blog.id}
			className={`flex max-h-full flex-col overflow-hidden rounded bg-white ring-1 ring-slate-300 dark:bg-slate-900 dark:ring-slate-700 md:max-h-64 md:flex-row ${className}`}
		>
			<img
				src={blog.image}
				alt={blog.title}
